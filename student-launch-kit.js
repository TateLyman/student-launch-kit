...
function analyze() {
  ...
  const usesSupabase = hasDependency(pkg, ['@supabase/supabase-js']) || /\bcreateClient\(|supabaseUrl|SUPABASE_URL|from\(['"][a-z0-9_]+['"]\)/i.test(allText)
  ...
  const usesFirebase = hasDependency(pkg, ['firebase', 'firebase-admin']) || /initializeApp\(|getFirestore\(|firebaseConfig|FIREBASE_/i.test(allText)
  const hasFirebaseRules = hasFile(entries, name => /(?:^|\/)(firestore\.rules|storage\.rules)$/.test(name))
  const paidApi = hasDependency(pkg, ['openai', '@anthropic-ai/sdk', '@google/generative-ai', 'stripe'])
    || /\b(?:OPENAI_API_KEY|ANTHROPIC_API_KEY|GEMINI_API_KEY|STRIPE_SECRET_KEY|new OpenAI\(|new Anthropic\(|checkout\.sessions)\b/i.test(allText)
